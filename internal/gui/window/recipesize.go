...
func (r *Recipe) sizeWayFor(b *batch, at func(string) string,
	...
	names := sizeWayNames()
	if b.sizeWay == nil {
		b.sizeWay = widget.NewRadioGroup(names, nil)
		b.sizeWay.Horizontal = true
		// Required, so there is no fourth state where nothing is chosen and no
		// box is shown. One of the three is always the answer.
		b.sizeWay.Required = true
		b.sizeWay.Selected = names[0]
	}
