...
var modules = []Module{
	...
	{Path: "github.com/go-gl/gl", SPDX: "MIT", Copyright: "(c) 2014 Eric Woroshow",
		Note: "The OpenGL binding. Carried as a copy under third_party/go-gl-gl since 2026-09-17, with one change: on Windows it looks wglGetProcAddress up at run time instead of importing it, so the window binary carries no load time import of opengl32.dll and can fall back to a software renderer. The version is the one go.mod requires and the build reports, and the copy is held to that version plus exactly the change by a guard - see PATCH.md there."},
