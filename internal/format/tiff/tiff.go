...
func chooseSize(r format.Request) (int, int, error) {
	...
	if wSet || hSet {
		w, err := imagedim.Value("tiff", imagedim.SettingWidth, r.Properties, maxDimension, 0)
		if err != nil {
			return 0, 0, err
		}
		h, err := imagedim.Value("tiff", imagedim.SettingHeight, r.Properties, maxDimension, 0)
		if err != nil {
			return 0, 0, err
		}
		switch {
