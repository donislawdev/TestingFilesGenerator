...
func namedSize(r format.Request, label string, q int) (memo, error) {
	w, err := imagedim.Value("jxl", imagedim.SettingWidth, r.Properties, maxDimension, sizeLadder[0].width)
	if err != nil {
		return memo{}, err
	}
	h, err := imagedim.Value("jxl", imagedim.SettingHeight, r.Properties, maxDimension, sizeLadder[0].height)
	if err != nil {
		return memo{}, err
	}
