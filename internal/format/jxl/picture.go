// The pixels: what a JPEG XL from this tool actually shows.
//
// The same gradient the other picture formats draw, so a tester comparing a
// PNG, a JPG, an AVIF and a JXL of the same size is looking at the same image
// four ways. The whole picture is built in memory before anything is encoded,
// because the encoder takes an image rather than a row at a time - which is
// what the megapixel limit in jxl.go is for.
package jxl
