...
// again at v0.45.0 on 2026-09-02 and again at v0.46.0 on 2026-09-22, each time
// the module was raised - a claim about what somebody else ships has to be
// re-read when their version moves, not carried across with the number
// changed. Pure Go
...
func chooseSize(r format.Request) (int, int, error) {
	...
	if wSet || hSet {
		w, err := imagedim.Value("webp", imagedim.SettingWidth, r.Properties, maxDimension, 0)
		if err != nil {
			return 0, 0, err
		}
		h, err := imagedim.Value("webp", imagedim.SettingHeight, r.Properties, maxDimension, 0)
		if err != nil {
			return 0, 0, err
		}
		switch {
